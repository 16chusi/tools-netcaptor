...
type AIService struct {
	models         []AIModel
	proxyConfigMgr interface{}
	smartProxyMgr  interface{}
}

// NewAIService 创建AI服务
func NewAIService(proxyConfigMgr interface{}, smartProxyMgr interface{}) *AIService {
	// 添加默认模型配置，避免模型索引超出范围
	...
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 初始化AI服务，默认模型数量: %d", len(defaultModels)))
...
func (s *AIService) CallAIWithImages(modelIndex int, prompt string, imageUrls []string, customSettings *AICustomSettings) (string, error) {
	utils.AppLog.Debug(fmt.Sprintf("[AI服务] 图片处理请求 - 模型索引: %d, 可用模型数量: %d", modelIndex, len(s.models)))

	if len(s.models) == 0 {
		utils.AppLog.Error("[AI服务] ❌ 没有配置任何AI模型")
		return "", fmt.Errorf("没有配置AI模型，请先在设置中配置AI模型")
	}

	utils.AppLog.Debug(fmt.Sprintf("[AI服务] 可用模型列表:"))
	for i, model := range s.models {
		utils.AppLog.Debug(fmt.Sprintf("[AI服务] 模型 %d: %s (%s)", i, model.Name, model.Provider))
	}

	if modelIndex >= len(s.models) {
		utils.AppLog.Error(fmt.Sprintf("[AI服务] ❌ 模型索引 %d 超出范围，可用模型: %d 个", modelIndex, len(s.models)))
...
func (s *AIService) CallAIWithCustomSettings(modelIndex int, prompt string, systemPrompt string, retryCount int, retryDelay int, customSettings *AICustomSettings) (string, error) {
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 文本处理请求 - 模型索引: %d, 可用模型数量: %d", modelIndex, len(s.models)))

	if len(s.models) == 0 {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] ❌ 没有配置任何AI模型"))
		return "", fmt.Errorf("没有配置AI模型，请先在设置中配置AI模型")
	}

	if modelIndex >= len(s.models) {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] ❌ 模型索引 %d 超出范围，可用模型: %d 个", modelIndex, len(s.models)))
		return "", fmt.Errorf("模型索引超出范围，请选择有效的模型 (0-%d)", len(s.models)-1)
	}

	model := s.models[modelIndex]
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 使用模型: %s (%s)", model.Name, model.Provider))
	...
	for attempt := 0; attempt <= retryCount; attempt++ {
		if attempt > 0 {
			...
			utils.AppLog.Info(fmt.Sprintf("[AI服务] 第 %d 次重试，等待 %v...", attempt, waitTime))
			time.Sleep(waitTime)
		}

		utils.AppLog.Info(fmt.Sprintf("[AI服务] 开始第 %d 次尝试调用AI", attempt+1))
		...
		if err == nil {
			if attempt > 0 {
				utils.AppLog.Info(fmt.Sprintf("[AI服务] ✅ 重试成功，第 %d 次尝试", attempt+1))
			} else {
				utils.AppLog.Info(fmt.Sprintf("[AI服务] ✅ 首次调用成功"))
			}
			return result, nil
		}

		lastErr = err
		utils.AppLog.Info(fmt.Sprintf("[AI服务] ❌ 第 %d 次尝试失败: %v", attempt+1, err))

		// 检查是否是可重试的错误
		if !isRetryableError(err) {
			utils.AppLog.Info(fmt.Sprintf("[AI服务] 不可重试的错误，停止重试"))
			break
		}
	}
...
func (s *AIService) makeHTTPRequest(url, apiKey string, request AIRequest, authType string) (string, error) {
	...
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 请求参数: %s", string(jsonData)))
	...
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 发送请求到: %s (超时: %v)", url, timeout))

	// 类型断言
	var client *http.Client
	if proxyMgr, ok := s.proxyConfigMgr.(*proxy.ProxyConfigManager); ok {
		if smartMgr, ok := s.smartProxyMgr.(*proxy.SmartProxyManager); ok {
			client = proxyMgr.CreateSmartHTTPClient(apiURL, smartMgr, timeout)
		} else {
	...
	if err != nil {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] 请求失败: %v", err))
		return "", err
	}
	...
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 收到响应，状态码: %d", resp.StatusCode))

	if resp.StatusCode != http.StatusOK {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] API调用失败，响应: %s", string(body)))
		return "", fmt.Errorf("API调用失败 (状态码: %d): %s", resp.StatusCode, string(body))
	}

	var response AIResponse
	if err := json.Unmarshal(body, &response); err != nil {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] 响应解析失败: %v, 原始响应: %s", err, string(body)))
		return "", fmt.Errorf("响应解析失败: %v", err)
	}

	if len(response.Choices) > 0 {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] 成功获取AI响应"))
...
func (s *AIService) UpdateModels(models []AIModel) {
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 更新模型配置，新模型数量: %d", len(models)))
	s.models = models

	// 如果没有配置模型，保留默认模型
	if len(s.models) == 0 {
		utils.AppLog.Warning("[AI服务] ⚠️ 没有提供模型配置，保持默认模型")
	} else {
		for i, model := range s.models {
			utils.AppLog.Debug(fmt.Sprintf("[AI服务] 模型 %d: %s (%s)", i, model.Name, model.Provider))
		}
...
func (s *AIService) TestModel(model AIModel) error {
	utils.AppLog.Info(fmt.Sprintf("[AI服务] 测试模型连接: %s", model.Name))
	...
	if err != nil {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] API调用失败: %v", err))
	} else {
		utils.AppLog.Info(fmt.Sprintf("[AI服务] API调用成功"))
	}
