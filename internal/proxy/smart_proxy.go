...
import "netcaptor/internal/utils"
import (
	"fmt"
	"strings"
	"time"
...
func (sm *SmartProxyManager) LoadRules() error {
	...
	err := cm.GetConfigJSON("smart_proxy_rules", &sm.rules)
	if err != nil || len(sm.rules) == 0 {
		// 添加默认规则
		sm.addDefaultRules()
		return sm.SaveRules()
	}
	utils.AppLog.Info(fmt.Sprintf("[SmartProxy] 加载了 %d 条路由规则", len(sm.rules)))
	return nil
}
...
func (sm *SmartProxyManager) SaveRules() error {
	cm := utils.GetConfigManager()
	return cm.SetConfigJSON("smart_proxy_rules", sm.rules)
}
...
func (sm *SmartProxyManager) DecideRoute(host string) string {
	...
	if rule != nil && rule.Enabled {
		...
		utils.AppLog.Info(fmt.Sprintf("[SmartProxy] %s 匹配规则: %s -> %s (%s)", host, rule.Pattern, rule.RouteType, rule.Source))
		return rule.RouteType
	}

	// 没有匹配规则，默认直连
	utils.AppLog.Info(fmt.Sprintf("[SmartProxy] %s 无匹配规则，使用直连", host))
	return "direct"
}
...
func (sm *SmartProxyManager) LearnFromFailure(host string, reason string) {
	...
	utils.AppLog.Info(fmt.Sprintf("[SmartProxy] 自动学习: %s 直连失败(%s)，添加代理规则", host, reason))
}
...
func (sm *SmartProxyManager) AddManualRule(pattern, routeType string) error {
	...
	sm.rules = append(sm.rules, rule)
	utils.AppLog.Info(fmt.Sprintf("[SmartProxy] 添加手动规则: %s -> %s", pattern, routeType))
	return sm.SaveRules()
}
...
func (sm *SmartProxyManager) RemoveRule(id string) error {
	for i, rule := range sm.rules {
		if rule.ID == id {
			...
			utils.AppLog.Info(fmt.Sprintf("[SmartProxy] 删除规则: %s", rule.Pattern))
			return sm.SaveRules()
		}
	}
...
func (sm *SmartProxyManager) ClearAutoLearnedRules() error {
	...
	utils.AppLog.Info(fmt.Sprintf("[SmartProxy] 清空了 %d 条自动学习规则", removed))
	return sm.SaveRules()
}
