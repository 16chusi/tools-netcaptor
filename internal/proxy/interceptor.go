...
func (i *Interceptor) InterceptResponse(resp *http.Response, req *http.Request) error {
	...
	for _, rule := range i.rules {
		...
		utils.AppLog.Info(fmt.Sprintf("[Interceptor] ✓ 匹配到规则: %s, ActionType: %s, SaveFilePath: %s", rule.Name, rule.ActionType, rule.SaveFilePath))
		...
		if (rule.ActionType == "saveToFile" || rule.SaveToFile) && rule.SaveFilePath != "" {
			...
			utils.AppLog.Info(fmt.Sprintf("[Interceptor] 未触发保存: ActionType=%s, SaveToFile=%v, SaveFilePath=%s", rule.ActionType, rule.SaveToFile, rule.SaveFilePath))
		}
