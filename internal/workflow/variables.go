...
func (we *WorkflowExecutor) replaceVariablesInString(str string) string {
	...
	if str != original {
		utils.AppLog.Debug(fmt.Sprintf("[Workflow] 变量替换: %s -> %s", original, str))
	}
	return str
}
...
func (we *WorkflowExecutor) resolveVariablePath(path string) interface{} {
	utils.AppLog.Debug(fmt.Sprintf("[Workflow] resolveVariablePath: %s", path))

	if strings.Contains(path, ".") {
		parts := strings.SplitN(path, ".", 2)
		utils.AppLog.Debug(fmt.Sprintf("[Workflow] 解析嵌套路径: 变量=%s, 字段=%s", parts[0], parts[1]))

		if val, ok := we.variables[parts[0]]; ok {
			utils.AppLog.Debug(fmt.Sprintf("[Workflow] 变量存在，类型: %T, 值: %v", parts[0], val, val))

			// 如果是 map，直接访问
			if mapVal, isMap := val.(map[string]interface{}); isMap {
				utils.AppLog.Info(fmt.Sprintf("[Workflow] 变量是 map，访问字段: %s", parts[1]))
				if result, exists := mapVal[parts[1]]; exists {
					utils.AppLog.Info(fmt.Sprintf("[Workflow] ✓ 找到字段 %s: %v", parts[1], result))
					return result
				}
				utils.AppLog.Info(fmt.Sprintf("[Workflow] ✗ map 中不存在字段: %s", parts[1]))
			}

			// 如果是 JSON 字符串，尝试解析
			if strVal, isStr := val.(string); isStr {
				utils.AppLog.Info(fmt.Sprintf("[Workflow] 变量是字符串，尝试解析 JSON"))
				var jsonData map[string]interface{}
				if err := json.Unmarshal([]byte(strVal), &jsonData); err == nil {
					utils.AppLog.Info(fmt.Sprintf("[Workflow] JSON 解析成功，访问字段: %s", parts[1]))
					if result, exists := jsonData[parts[1]]; exists {
						utils.AppLog.Info(fmt.Sprintf("[Workflow] ✓ 找到字段 %s: %v", parts[1], result))
						return result
					}
					utils.AppLog.Info(fmt.Sprintf("[Workflow] ✗ JSON 中不存在字段: %s", parts[1]))
				} else {
					utils.AppLog.Info(fmt.Sprintf("[Workflow] JSON 解析失败: %v", err))
				}
			}
		} else {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] ✗ 变量 %s 不存在", parts[0]))
		}
		...
		utils.AppLog.Info(fmt.Sprintf("[Workflow] 直接访问变量: %s", path))
		if val, ok := we.variables[path]; ok {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] ✓ 变量存在，类型: %T", val))
			return val
		}
		utils.AppLog.Info(fmt.Sprintf("[Workflow] ✗ 变量不存在: %s", path))
	}
	return nil
}
...
func (we *WorkflowExecutor) setVariable(name string, value interface{}) {
	...
	utils.AppLog.Debug(fmt.Sprintf("[Workflow] 设置变量 %s = %v", name, value))
}
