...
func (we *WorkflowExecutor) executeIf(step ExecutionStep, task WorkflowTask) (string, error) {
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] 评估条件: %s %s %s", leftValue, operator, rightValue))
	...
	if result {
		targetPort = truePort
		utils.AppLog.Info(fmt.Sprintf("[Workflow] 条件为 true，选择端口: %s", targetPort))
	} else {
		utils.AppLog.Info(fmt.Sprintf("[Workflow] 条件为 false，选择端口: %s", targetPort))
	}
	...
	for _, edge := range task.Edges {
		...
		if edge.Source == step.NodeID && edgeSourcePort == targetPort {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 找到匹配的边: %s -> %s (端口: %s)", edge.Source, edge.Target, targetPort))
