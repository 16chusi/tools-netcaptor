...
func (we *WorkflowExecutor) executeNavigate(step ExecutionStep) (ExecutionResult, error) {
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] 导航到: %s", url))
	utils.AppLog.Info(fmt.Sprintf("[Workflow] 当前变量: %+v", we.variables))
...
func (we *WorkflowExecutor) executeClick(step ExecutionStep) (ExecutionResult, error) {
	utils.AppLog.Debug(fmt.Sprintf("[Action] 开始执行"))
	selector, ok := step.Params["selector"].(string)
	if !ok || selector == "" {
		utils.AppLog.Info(fmt.Sprintf("[点击元素节点] 错误: 缺少 selector 参数"))
	...
	utils.AppLog.Debug(fmt.Sprintf("[Action] 执行参数: selector=%s, selectorType=%s", selector, selectorType))
	...
	if err != nil {
		utils.AppLog.Info(fmt.Sprintf("[点击元素节点] 执行失败: %v", err))
	} else {
		utils.AppLog.Info(fmt.Sprintf("[点击元素节点] 执行成功"))
	}
	return result, err
}
...
func (we *WorkflowExecutor) executeInput(step ExecutionStep) (ExecutionResult, error) {
	utils.AppLog.Debug(fmt.Sprintf("[Action] 开始执行"))
	utils.AppLog.Debug(fmt.Sprintf("[Action] 参数: %+v", step.Params))

	// 检查WebSocket连接状态
	if !we.wsServer.IsRunning() {
		utils.AppLog.Info(fmt.Sprintf("[输入文本节点] WebSocket服务器未运行"))
		return ExecutionResult{Success: false}, fmt.Errorf("WebSocket服务器未运行")
	}
	if !we.wsServer.HasClients() {
		utils.AppLog.Info(fmt.Sprintf("[输入文本节点] 没有浏览器扩展连接"))
		return ExecutionResult{Success: false}, fmt.Errorf("没有浏览器扩展连接")
	}
	utils.AppLog.Info(fmt.Sprintf("[输入文本节点] WebSocket连接正常"))

	selector, ok := step.Params["selector"].(string)
	if !ok || selector == "" {
		utils.AppLog.Info(fmt.Sprintf("[输入文本节点] 错误: 缺少 selector 参数"))
		return ExecutionResult{Success: false}, fmt.Errorf("缺少 selector 参数")
	}

	text, ok := step.Params["text"].(string)
	if !ok {
		utils.AppLog.Info(fmt.Sprintf("[输入文本节点] 错误: 缺少 text 参数"))
	...
	utils.AppLog.Debug(fmt.Sprintf("[Action] 执行参数: selector=%s, text=%s, selectorType=%s", selector, text, selectorType))
	...
	utils.AppLog.Info(fmt.Sprintf("[输入文本节点] 发送WebSocket消息"))
	result, err := we.sendAndWait(msg, 30*time.Second, "input_text")
	if err != nil {
		utils.AppLog.Info(fmt.Sprintf("[输入文本节点] 执行失败: %v", err))
	} else {
		utils.AppLog.Info(fmt.Sprintf("[输入文本节点] 执行成功"))
	}
...
func (we *WorkflowExecutor) executeExtract(step ExecutionStep) (ExecutionResult, error) {
	...
	if selector == "" {
		utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 错误: 缺少 selector 参数"))
		return ExecutionResult{Success: false}, fmt.Errorf("缺少 selector 参数")
	}
	...
	utils.AppLog.Debug(fmt.Sprintf("[Action] 执行参数: selector=%s, selectorType=%s, attribute=%s", selector, selectorType, attribute))
	...
	utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 发送WebSocket消息到浏览器扩展: %+v", msg))
	utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 等待浏览器扩展响应..."))

	result, err := we.sendAndWait(msg, 30*time.Second, "extract")

	if err == nil && result.Success {
		utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 收到成功响应: %+v", result.Data))
		if saveToVariable, ok := step.Params["saveToVariable"].(string); ok && saveToVariable != "" {
			if data, ok := result.Data["value"]; ok {
				...
				utils.AppLog.Debug(fmt.Sprintf("[获取网页内容节点] 准备保存数据到变量 %s (原始: %s): %v", cleanSaveToVariable, saveToVariable, data))

				// 检查是否覆盖了现有变量
				if oldValue, exists := we.variables[cleanSaveToVariable]; exists {
					utils.AppLog.Debug(fmt.Sprintf("[获取网页内容节点] ⚠️ 覆盖现有变量 %s: 旧值=%v, 新值=%v", cleanSaveToVariable, oldValue, data))
				}

				we.variables[cleanSaveToVariable] = data
				utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] ✓ 数据已保存到变量: %s = %v", cleanSaveToVariable, data))

				// 验证保存是否成功
				if savedValue, exists := we.variables[cleanSaveToVariable]; exists {
					utils.AppLog.Debug(fmt.Sprintf("[获取网页内容节点] ✓ 验证保存成功: %s = %v", cleanSaveToVariable, savedValue))
				} else {
					utils.AppLog.Error(fmt.Sprintf("[获取网页内容节点] ❌ 验证保存失败: 变量 %s 不存在", cleanSaveToVariable))
				}
			} else {
				utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] ❌ 响应中没有 value 字段: %+v", result.Data))
			}
		} else {
			utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 没有配置 saveToVariable 参数，不保存数据"))
		}
		utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 执行成功"))
	} else {
		utils.AppLog.Info(fmt.Sprintf("[获取网页内容节点] 执行失败: %v", err))
	}
...
func (we *WorkflowExecutor) executeScroll(step ExecutionStep) (ExecutionResult, error) {
	...
	if scrollType == "times" {
		if times, ok := step.Params["times"].(float64); ok {
			msg.Data.(map[string]interface{})["times"] = int(times)
		}
	} else if scrollType == "distance" {
		if distance, ok := step.Params["distance"].(float64); ok {
			msg.Data.(map[string]interface{})["distance"] = int(distance)
		}
	}
