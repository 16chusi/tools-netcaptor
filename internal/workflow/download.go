...
func (we *WorkflowExecutor) executeDownload(step ExecutionStep) (ExecutionResult, error) {
	...
	for i, url := range urls {
		...
		if _, err := os.Stat(savePath); err == nil {
			...
			uuidStr := fmt.Sprintf("%d", time.Now().UnixNano())[:8]
			filename = fmt.Sprintf("%s_%s%s", base, uuidStr, ext)
			savePath = filepath.Join(saveDirectory, filename)
		}

		utils.AppLog.Info(fmt.Sprintf("[Workflow] 下载文件 %d/%d: %s -> %s", i+1, len(urls), url, savePath))

		resp, err := http.Get(url)
		if err != nil {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 下载失败: %v", err))
			continue
		}
		defer resp.Body.Close()

		if resp.StatusCode != http.StatusOK {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 下载失败: HTTP %d", resp.StatusCode))
			continue
		}

		file, err := os.Create(savePath)
		if err != nil {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 创建文件失败: %v", err))
			continue
		}
		defer file.Close()
		...
		if err != nil {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 写入文件失败: %v", err))
			os.Remove(savePath)
			continue
		}

		utils.AppLog.Info(fmt.Sprintf("[Workflow] ✓ 下载成功: %s", filename))
