...
func (we *WorkflowExecutor) executeJSONLReaderWithDepth(step ExecutionStep, task WorkflowTask, stepCount int, depth int) error {
	utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] ========== 开始执行 (深度:%d) ==========", depth))
	utils.AppLog.Debug(fmt.Sprintf("[Action] 参数: %+v", step.Params))

	// 获取文件路径
	filePath, ok := step.Params["filePath"].(string)
	if !ok || filePath == "" {
		utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] 错误: 缺少 filePath 参数"))
		return fmt.Errorf("缺少 filePath 参数")
	}

	utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] 文件路径: %s", filePath))
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] JSONL读取器: 文件=%s, 字段=%v, 变量=%s, 间隔=%dms, 最大次数=%d", filePath, extractKeys, saveToVariable, loopInterval, maxCount))
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] JSONL文件加载成功，共 %d 行", lineCount))
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] 将处理 %d 行数据", actualLoops))
	...
	for i := 0; i < actualLoops; i++ {
		...
		utils.AppLog.Debug(fmt.Sprintf("[JSONL] ========== 处理第 %d/%d 行 ==========", i+1, lineCount))

		// 每次循环前等待，确保页面和扩展状态稳定
		if i > 0 { // 第一次不需要等待
			utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] 等待页面稳定..."))
		...
		utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] ✓ 变量已保存: %s = %v", saveToVariable, extractedData))

		utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] ========== 保存后的所有变量 =========="))
		for key, value := range we.variables {
			utils.AppLog.Debug(fmt.Sprintf("[Workflow] 变量: %s (%T) = %v", key, value, value))
		}
		utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] ====================================="))

		// 执行循环体（从下一个节点开始，直到结束节点）
		utils.AppLog.Info(fmt.Sprintf("[JSONL读取器] 准备执行循环体..."))
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] JSONL读取器执行完成，共处理 %d 行", actualLoops))
	return nil
}
...
func (we *WorkflowExecutor) executeLoopBodyWithDepth(task WorkflowTask, startNodeID string, baseStepCount int, depth int) error {
	...
	for {
		...
		// 如果是结束节点，退出
		if node.Type == "end" {
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 循环体到达结束节点"))
			break
		}
		...
		stepCount++
		utils.AppLog.Info(fmt.Sprintf("[Workflow] 循环体步骤 %d (深度:%d): %s", stepCount, depth, node.Type))
		...
		if node.Type == "for" {
			...
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 执行嵌套for循环 (深度:%d)", depth+1))
		...
		if node.Type == "jsonl_reader" {
			...
			utils.AppLog.Info(fmt.Sprintf("[Workflow] 执行嵌套JSONL读取器 (深度:%d)", depth+1))
		...
		utils.AppLog.Debug(fmt.Sprintf("[Workflow] ========== 当前变量状态 =========="))
		for key, value := range we.variables {
			utils.AppLog.Debug(fmt.Sprintf("[Workflow] 变量: %s (%T) = %v", key, value, value))
		}
		utils.AppLog.Info(fmt.Sprintf("[循环体执行器] ====================================="))
		...
		utils.AppLog.Info(fmt.Sprintf("[循环体执行器] 准备执行节点: [%s] ID=%s, 参数=%+v", nodeDisplayName, currentNodeID, step.Params))
		...
		utils.AppLog.Info(fmt.Sprintf("[循环体执行器] ✅ 节点执行成功: [%s] %v", nodeDisplayName, message))
