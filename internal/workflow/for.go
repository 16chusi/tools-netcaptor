...
func (we *WorkflowExecutor) executeForWithDepth(step ExecutionStep, task WorkflowTask, stepCount int, depth int) error {
	...
	loopCount := int(count)
	utils.AppLog.Info(fmt.Sprintf("[Workflow] For循环 (深度:%d): 次数=%d, 变量=%s, 间隔=%dms", depth, loopCount, variable, interval))
	...
	for i := 1; i <= loopCount; i++ {
		...
		utils.AppLog.Debug(fmt.Sprintf("[For] ========== 循环第 %d/%d 次 (深度:%d) ==========", i, loopCount, depth))

		we.variables[variable] = i
		utils.AppLog.Info(fmt.Sprintf("[Workflow] ✓ 变量已设置: %s = %d", variable, i))
	...
	utils.AppLog.Info(fmt.Sprintf("[Workflow] For循环执行完成 (深度:%d)，共循环 %d 次", depth, loopCount))
	return nil
}

func (we *WorkflowExecutor) printAllVariables() {
	utils.AppLog.Debug(fmt.Sprintf("[Workflow] ========== 当前所有变量 =========="))
