...
func getModelIndex(params map[string]interface{}) int {
	...
	utils.AppLog.Debug(fmt.Sprintf("[AI] 原始参数: %+v", params))

	if mi, ok := params["modelIndex"].(float64); ok {
		index = int(mi)
		utils.AppLog.Debug(fmt.Sprintf("[AI] 从 float64 获取模型索引: %d", index))
	} else if mis, ok := params["modelIndex"].(string); ok {
		if idx, err := strconv.Atoi(mis); err == nil {
			index = idx
			utils.AppLog.Debug(fmt.Sprintf("[AI] 从 string 获取模型索引: %d", index))
		} else {
			utils.AppLog.Error(fmt.Sprintf("[AI] 字符串转换失败: %s, 错误: %v", mis, err))
		}
	} else {
		utils.AppLog.Error(fmt.Sprintf("[AI] 无法获取 modelIndex 参数，类型: %T, 值: %v", params["modelIndex"], params["modelIndex"]))
	}

	utils.AppLog.Info(fmt.Sprintf("[AI] 最终使用的模型索引: %d", index))
	return index
}
...
func (we *WorkflowExecutor) executeAIExtractData(step ExecutionStep) (ExecutionResult, error) {
	...
	if contentType == "image" {
		// 图片内容的特殊处理
		utils.AppLog.Info(fmt.Sprintf("[AI] 处理图片内容..."))
		...
		utils.AppLog.Info(fmt.Sprintf("[AI] 找到 %d 张图片", len(imageUrls)))

		// 使用自定义设置调用AI处理图片
		customSettings := &ai.AICustomSettings{}
...
func (we *WorkflowExecutor) getCurrentPageDOM(timeout int, contentType string) (string, error) {
	utils.AppLog.Info(fmt.Sprintf("[AI] 开始获取页面DOM，类型: %s", contentType))

	// 检查WebSocket连接状态
	if !we.wsServer.IsRunning() {
		utils.AppLog.Info(fmt.Sprintf("[AI] ❌ WebSocket服务器未运行"))
		return "", fmt.Errorf("WebSocket服务器未运行")
	}
	if !we.wsServer.HasClients() {
		utils.AppLog.Info(fmt.Sprintf("[AI] ❌ 没有浏览器扩展连接"))
	...
	utils.AppLog.Debug(fmt.Sprintf("[WebSocket] 发送WebSocket消息: %+v", message))
	result, err := we.sendAndWait(message, time.Duration(timeout)*time.Second, "get_page_dom")
	if err != nil {
		utils.AppLog.Info(fmt.Sprintf("[AI] ❌ WebSocket通信失败: %v", err))
		return "", err
	}

	utils.AppLog.Info(fmt.Sprintf("[AI] ✅ 收到WebSocket响应: Success=%v", result.Success))
	if !result.Success {
		utils.AppLog.Info(fmt.Sprintf("[AI] ❌ DOM获取失败: %v", result.Error))
	...
	if html, ok := result.Data["html"].(string); ok {
		...
		utils.AppLog.Info(fmt.Sprintf("[AI] ✅ 成功获取页面内容，类型: %s，长度: %d 字符", actualType, contentSize))
		return html, nil
	}

	utils.AppLog.Info(fmt.Sprintf("[AI] ❌ DOM数据格式错误: %+v", result.Data))
