...
func NewProxyConfigManager() *ProxyConfigManager {
	...
	configPath := filepath.Join(configDir, "proxy_config.json")
...
func (pcm *ProxyConfigManager) LoadConfig() error {
	...
	data, err := ioutil.ReadFile(pcm.configPath)
	if err != nil {
		return fmt.Errorf("读取配置文件失败: %w", err)
	}

	err = json.Unmarshal(data, pcm.config)
	if err != nil {
...
func (pcm *ProxyConfigManager) SaveConfig() error {
	...
	err = ioutil.WriteFile(pcm.configPath, data, 0644)
	if err != nil {
